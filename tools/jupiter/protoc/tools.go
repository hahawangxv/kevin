...
func Run(cli *cli.Context) (err error) {
	...
	if option.protoFilePath == "" {
		err = errors.New("Please specify the proto file path and use jupiter protoc - h to views the detailed prompt")
		return
	}
	if option.outputDir == "" {
		err = errors.New("Please specify the code generation path and use jupiter protoc - h to views the detailed prompt")
		return
	}
	// 默认生成全部
