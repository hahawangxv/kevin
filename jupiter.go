...
import (
	...
	file_datasource "github.com/hahawangxv/kevin/pkg/datasource/file"
	http_datasource "github.com/hahawangxv/kevin/pkg/datasource/http"
	...
	"github.com/hahawangxv/kevin/pkg/sentinel"
	"github.com/hahawangxv/kevin/pkg/server"
	"github.com/hahawangxv/kevin/pkg/trace"
	"github.com/hahawangxv/kevin/pkg/trace/jaeger"
...
// - load config
// - customer default biz logger, jupiter frame logger
// - customer procs
...
func (app *Application) startGovernor() (err error) {
	...
	// err = app.governor.ListenAndServeTLS("../config/www.idosp.cn_server.crt", "../config/www.idosp.cn_server.key")
...
func (app *Application) loadConfig() error {
	...
	if len(configAddr) == 0 {
		...
		APP_NAME = strings.TrimSuffix(APP_NAME, ".exe")
		configAddr = "../config/" + APP_NAME + ".toml"
	}
	if configAddr == "" {
	...
	templateConfigAddr := "../config/template.toml"
	...
	case strings.HasPrefix(configAddr, "http://"),
		...
		template := file_datasource.NewDataSource(templateConfigAddr, watchConfig)
		if err := conf.LoadFromDataSource(toml.Unmarshal, provider, template); err != nil {
			//templateConfigAddr := "./config/template.toml"
			//template = file_datasource.NewDataSource(templateConfigAddr, watchConfig)
...
func (app *Application) initTracer() error {
	// customer tracing component jaeger
...
func (app *Application) initSentinel() error {
	// customer reliability component sentinel
	if conf.Get("jupiter.reliability.sentinel") != nil {
		app.logger.Info("customer reliability component sentinel")
...
func (app *Application) printBanner() error {
	const banner = `
 Welcome to IdospApi, starting application ...
`
