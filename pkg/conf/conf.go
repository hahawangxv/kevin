...
import (
	...
	"github.com/hahawangxv/kevin/pkg/util/xcast"
	"github.com/hahawangxv/kevin/pkg/util/xmap"
...
// LoadFromDataSource ...
func (c *Configuration) LoadFromDataSource( unmarshaller Unmarshaller,dslist ... DataSource) error {
	var err error
	content, err := dslist[0].ReadConfig()
	if err != nil {
		return err
	}
	content2, err := dslist[1].ReadConfig()
	content = append(append(content,byte('\n')),content2...)
	...
	go func() {
		for range dslist[0].IsConfigChanged() {
			if content, err := dslist[0].ReadConfig(); err == nil {
